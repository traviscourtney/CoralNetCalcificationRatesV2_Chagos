...
ReefBudget_Pacific <- read_excel("CHAGOS Carbonate Production_v1.3.xlsx", sheet = "Calcification Rates", skip = 9) %>%
  select(!`vertical growth (cm/yr)`) %>% 
  select(!`CI...7`)#import Chagos ReefBudget rates
macromicro_bioerosion=read_excel("CHAGOS Carbonate Production_v1.3.xlsx", sheet = "Macro- & Microbioerosion") #import Indo_Pacific ReefBudget bioerosion rates
...
NOAA_mgr_all[NOAA_mgr_all$TAXA=="Pocillopora spp",10]=mean(subset(H_rugosities,GENUS=="Pocillopora")$rugosity)
NOAA_mgr_all[NOAA_mgr_all$TAXA=="Pocillopora spp",11]=sd(subset(H_rugosities,GENUS=="Pocillopora")$rugosity)
...
ReefBudget_Pacific_acropora_branching_rates =
  ...
  summarize(coeff_mean=mean(coeff_mean,na.rm=TRUE),
            coeff_lwr=mean(coeff_lwr,na.rm=TRUE),
            coeff_upr=mean(coeff_upr,na.rm=TRUE),
            int_mean=round(mean(int_mean,na.rm=TRUE),0),
            int_lwr=round(mean(int_lwr,na.rm=TRUE),0), 
            int_upr=round(mean(int_upr,na.rm=TRUE),0)) 
...
ReefBudget_Pacific_plerogyra_branching_rates =
  ...
  summarize(coeff_mean=mean(coeff_mean,na.rm=TRUE),
            ...
            int_lwr=mean(int_lwr,na.rm=TRUE), 
            int_upr=mean(int_upr,na.rm=TRUE)) 
...
file_name=ifelse(include_bioerosion==TRUE,"CoralNet_Chagos_Calcification_With_Bioerosion_Rates_v2.csv","CoralNet_Chagos_Calcification_Without_Bioerosion_Rates_v2.csv")
